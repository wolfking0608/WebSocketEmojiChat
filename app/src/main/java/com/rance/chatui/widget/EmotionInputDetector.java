...
import com.aking.websocket.WebSocketServiceConnectManager;
   ...
   private  WebSocketServiceConnectManager mConnectManager;
    ...
    public EmotionInputDetector bindToEmotionButton(View emotionButton) {
        emotionButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {//表情按钮的点击事件
    ...
    public EmotionInputDetector bindToSendButton(View sendButton) {
        ...
        sendButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {//发送事件的处理
                ...
                HistoryBean bean =new HistoryBean();//直接添加一个bean
                bean.setTime(String.valueOf(System.currentTimeMillis()/1000));
                bean.setData(mEditText.getText().toString());
                messageInfo.setContent(bean);
                sendText(mEditText.getText().toString());//webSocket发送事件的处理
    ...
    public EmotionInputDetector build() {//这里还不是好理解
    ...
    public void sendText(String text) {
        ...
        parameters.put("name","seanyou");
        parameters.put("level","100");
        parameters.put("user_id","957144");

        command.put("time",System.currentTimeMillis()/1000);
        command.put("type","text"); //这个要自己去判断
        command.put("cmd","message");
        command.put("data",text);
        command.put("user",parameters);
